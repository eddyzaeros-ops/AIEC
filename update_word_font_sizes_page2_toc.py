...
def add_pure_native_auto_toc(doc, instruction='TOC \\o "1-3" \\h \\z \\u'):
    # Word Native Auto-TOC element (w:sdt Component)
    sdt_xml = f'''
    <w:sdt {nsdecls("w")}>
      <w:sdtPr>
        <w:docPartObj>
...
def add_heading_styled(doc, text, level):
    # User Rules: Heading 1 = 20pt, Heading 2 = 16pt, Heading 3 = 14pt (bold)
    ...
    if level == 1:
        ...
        set_run_fonts(run, font_size=Pt(16), bold=True, color_rgb=RGBColor(37, 99, 235))
    elif level == 3:
        set_run_fonts(run, font_size=Pt(14), bold=True, color_rgb=RGBColor(30, 58, 138))
    return p

def add_caption_styled(doc, label_text, seq_name, title_text):
    ...
    set_run_fonts(r_lbl, font_size=Pt(11), bold=True, color_rgb=RGBColor(12, 35, 64))
    
    # Native SEQ Field
    fldSimple = parse_xml(f'<w:fldSimple {nsdecls("w")} w:instr="SEQ {seq_name} \\* ARABIC"/>')
    p._p.append(fldSimple)
    
    r_title = p.add_run("：" + title_text)
    set_run_fonts(r_title, font_size=Pt(11), bold=True, color_rgb=RGBColor(12, 35, 64))
    return p

def add_body_p(doc, text, bold_prefix="", space_after=8):
    # User Rule: Normal Body text = 14pt
    ...
    if bold_prefix:
        ...
        set_run_fonts(r_b, font_size=Pt(14), bold=True, color_rgb=RGBColor(12, 35, 64))
        
    r = p.add_run(text)
    set_run_fonts(r, font_size=Pt(14), color_rgb=RGBColor(51, 65, 85))
    return p

def build_font_sizes_page2_toc_docx():
    ...
    # User Rule: Heading 1 = 20pt, Heading 2 = 16pt, Normal Body = 14pt
    set_style_font_and_color(doc, 'Normal', Pt(14), color_rgb=RGBColor(51, 65, 85))
    set_style_font_and_color(doc, 'Heading 1', Pt(20), bold=True, color_rgb=RGBColor(12, 35, 64))
    set_style_font_and_color(doc, 'Heading 2', Pt(16), bold=True, color_rgb=RGBColor(37, 99, 235))
    set_style_font_and_color(doc, 'Heading 3', Pt(14), bold=True, color_rgb=RGBColor(30, 58, 138))
    set_style_font_and_color(doc, 'Caption', Pt(11), bold=True, color_rgb=RGBColor(12, 35, 64))
    ...
    set_run_fonts(r_badge, font_size=Pt(16), bold=True, color_rgb=RGBColor(37, 99, 235))
    ...
    set_run_fonts(r_sub, font_size=Pt(14), bold=True, color_rgb=RGBColor(71, 85, 105))
    ...
    for idx, (k, v) in enumerate(meta_data):
        ...
        r0 = p0.add_run(k + "：")
        set_run_fonts(r0, font_size=Pt(12), bold=True, color_rgb=RGBColor(37, 99, 235))

        p1 = c1.paragraphs[0]
        r1 = p1.add_run(v)
        set_run_fonts(r1, font_size=Pt(12), color_rgb=RGBColor(12, 35, 64))

    # Page Break to Page 2
    doc.add_page_break()

    # ----------------------------------------------------
    # PAGE 2: AUTOMATIC TABLE OF CONTENTS (文件第二頁，插入自動目錄區域)
    # ----------------------------------------------------
    add_heading_styled(doc, "📋 目錄 (Table of Contents)", level=1)
    add_pure_native_auto_toc(doc, instruction='TOC \\o "1-3" \\h \\z \\u')

    doc.add_paragraph().paragraph_format.space_after = Pt(12)
    
    # 圖目錄 (List of Figures)
    add_heading_styled(doc, "🖼️ 圖目錄 (List of Figures)", level=2)
    add_pure_native_auto_toc(doc, instruction='TOC \\h \\z \\c "圖"')

    doc.add_paragraph().paragraph_format.space_after = Pt(12)
    
    # 表目錄 (List of Tables)
    add_heading_styled(doc, "📊 表目錄 (List of Tables)", level=2)
    add_pure_native_auto_toc(doc, instruction='TOC \\h \\z \\c "表"')

    # Page Break to Page 3 (Chapter 1)
    doc.add_page_break()

    # ----------------------------------------------------
    # PAGE 3 ONWARDS: CHAPTERS 1 TO 5
    # ----------------------------------------------------
    ...
    # Table 1-1 Caption
    add_caption_styled(doc, "表", "表", "傳統軍事戰術 vs. 非對稱 AI 戰術比較表")
    ...
    for j, h in enumerate(headers):
        ...
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        r = p.add_run(h)
        set_run_fonts(r, font_size=Pt(12), bold=True, color_rgb=RGBColor(255, 255, 255))
    ...
    for i, row_data in enumerate(rows_t1):
        ...
        for j, val in enumerate(row_data):
            ...
            if j == 0:
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            r = p.add_run(val)
            set_run_fonts(r, font_size=Pt(11.5), color_rgb=RGBColor(30, 41, 59))
    ...
    # Table 2-1 Caption
    add_caption_styled(doc, "表", "表", "ISO 42001 (AIMS) 通用標準 vs. AIEC 國防權責分工比較表")
    ...
    # Table 2-2 Caption
    add_caption_styled(doc, "表", "表", "交戰規則 (RoE) 人機授權三階權能邊界比較表 (HITL / HOTL / HOOTL)")
    ...
    for j, h in enumerate(["授權階層 (RoE Tier)", "人類角色 (Human Role)", "AI 系統權能範圍", "適用國防戰術情境"]):
        ...
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        r = p.add_run(h)
        set_run_fonts(r, font_size=Pt(11.5), bold=True, color_rgb=RGBColor(255, 255, 255))
    ...
    for i, row_data in enumerate(rows_t22):
        ...
        for j, val in enumerate(row_data):
            ...
            if j == 0:
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            r = p.add_run(val)
            set_run_fonts(r, font_size=Pt(11.0), color_rgb=RGBColor(30, 41, 59))

    # Figure 2-1 Image & Caption
    ...
    add_caption_styled(doc, "圖", "圖", "AIEC 國防 AI 評測五大階段標準流程圖 (Nano Banana pro 生成)")
    ...
    # Table 2-3 Caption
    add_caption_styled(doc, "表", "表", "AIEC 15 項量化評測指標 (Q1~Q15) 與合格門檻對照總表")
    ...
    for j, h in enumerate(["編號", "指標名稱 (英文縮寫)", "量化合格判定門檻 (PASS Threshold)", "代表性驗測工具鏈"]):
        ...
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        r = p.add_run(h)
        set_run_fonts(r, font_size=Pt(11), bold=True, color_rgb=RGBColor(255, 255, 255))
    ...
    for i, row_data in enumerate(q_data):
        ...
        for j, val in enumerate(row_data):
            ...
            if j in [0, 2]:
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            r = p.add_run(val)
            set_run_fonts(r, font_size=Pt(10), color_rgb=RGBColor(30, 41, 59))
    ...
    add_caption_styled(doc, "圖", "圖", "國防 AI 安全保密審計 3D 縱深防禦矩陣圖 (Nano Banana pro 生成)")
    ...
    add_caption_styled(doc, "圖", "圖", "民雄院區國家級主權 AI 四層算力堆疊架構圖 (Nano Banana pro 生成)")
    ...
    # Table 3-1 Caption
    add_caption_styled(doc, "表", "表", "AIEC 國防 AI 導入專案 24 個月執行時程規劃表")
    ...
    # Table 4-1 Caption
    add_caption_styled(doc, "表", "表", "專案人力、主權算力與軟硬體資源配置表")
    ...
    for i, row_data in enumerate(res_data):
        ...
        for j, val in enumerate(row_data):
            ...
            if j == 0:
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            r = p.add_run(val)
            set_run_fonts(r, font_size=Pt(10.5), color_rgb=RGBColor(30, 41, 59))

    add_heading_styled(doc, "4.2 專案風險評估與因應對策 (Risk Matrix)", level=2)

    # Table 4-2 Caption
    add_caption_styled(doc, "表", "表", "國防 AI 導入主要風險評估與處置矩陣表")
    ...
    print(f"Successfully generated Word proposal document with Heading 1=20pt, Heading 2=16pt, Normal Body=14pt & Page 2 Auto-TOC at:\n - {out_local}\n - {out_desktop}")

if __name__ == '__main__':
    build_font_sizes_page2_toc_docx()
