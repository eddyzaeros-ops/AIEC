...
def generate_37_slides_master_deck():
    ...
    # ----------------------------------------------------
    # Slide 2: TABLE OF CONTENTS (Update Slide 2 for P.30 - 37)
    # ----------------------------------------------------
    ...
    c2_bullets = [
        ...
        "   - P.35 評測流程圖 | P.36 3D防禦矩陣 | P.37 四層算力堆疊 (高解析度圖表)"
    ]
    ...
    # ----------------------------------------------------
    # Slide 32: FIX LAYOUT (Requirement 1 - Clean Re-layout!)
    # ----------------------------------------------------
    ...
    # Fixed position for img_tau_abort under Bullet 1 at y=4.7 (Zero collision!)
    ...
    # ----------------------------------------------------
    # Infographic Slide 1: Slide 35 (Requirement 2: < 50 words description!)
    # ----------------------------------------------------
    ...
    add_icon_card(s35, 0.8, 1.4, 14.4, 1.4, "🗺️", "國防 AIEC 評測五大階段架構流程圖", "Five-Stage Evaluation Pipeline Flowchart", accent_color=BLUE)
    ...
    p35_desc = [
        "📌 **資訊圖表說明 (45 字)**：本圖展現 AIEC 國防 AI 評測 5 大階段標準流程，從需求與交戰規則 (RoE) 界定、數據清洗與 Provenance 溯源、Cyber Range 對抗演練，到 15 項量化評測與 TRL 戰術部署放行。"
    ]
    ...
    # ----------------------------------------------------
    # Infographic Slide 2: Slide 36 (Requirement 2: < 50 words description!)
    # ----------------------------------------------------
    ...
    add_icon_card(s36, 0.8, 1.4, 14.4, 1.4, "🛡️", "國防 AI 安全保密審計 3D 縱深防禦矩陣", "3D Defense Matrix (Security, Confidentiality & Auditing)", accent_color=DARK_BLUE)
    ...
    p36_desc = [
        "📌 **資訊圖表說明 (46 字)**：本圖呈現國防 AI 三維縱深防禦矩陣。整合 Security (ATLAS 對抗防禦與硬體自毀)、Confidentiality (地端實體隔離與聯邦學習) 與 Auditing (Data/Model Provenance 溯源與 XAI 特徵熱力圖)。"
    ]
    ...
    # ----------------------------------------------------
    # Infographic Slide 3: Slide 37 (Requirement 2: < 50 words description!)
    # ----------------------------------------------------
    ...
    add_icon_card(s37, 0.8, 1.4, 14.4, 1.4, "🏛️", "主權 AI 四層算力堆疊與邊緣部署架構圖", "Sovereign AI Four-Tier Compute Stack Pyramid", accent_color=BLUE)
    ...
    p37_desc = [
        "📌 **資訊圖表說明 (44 字)**：本圖展現民雄院區主權 AI 四層算力架構。由 Tier 1 密集型 70B 主權模型至 Tier 3 邊緣 7B/3B 蒸餾量化模型，搭配邊緣硬體自毀機制，達成極致算力與戰術保密。"
    ]
