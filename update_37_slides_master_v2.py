...
from generate_infographics import build_infographic_s35, build_infographic_s36, build_infographic_s37
...
ACRONYM_DB = {
    ...
    "RoE": ("交戰規則", "Rules of Engagement"),
    "TRL": ("技術成熟度等級", "Technology Readiness Level"),
...
def generate_37_slides_master_deck():
    # Build infographic PNGs
    ...
    # ----------------------------------------------------
    # Slide 1: TITLE SLIDE (Updated with CTO & Architect Perspective)
    # ----------------------------------------------------
    ...
    badge = s1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(1.3), Inches(1.0), Inches(8.5), Inches(0.55))
    badge.fill.solid()
    badge.fill.fore_color.rgb = BLUE
    badge.line.fill.background()
    p = badge.text_frame.paragraphs[0]
    p.text = "🛡️ 國防 AI 技術長暨架構師觀點 — AIEC 專屬評測與驗證 SOP 體系"
    ...
    tb1 = s1.shapes.add_textbox(Inches(1.3), Inches(1.8), Inches(13.4), Inches(2.0))
    tf1 = tb1.text_frame
    tf1.word_wrap = True
    p = tf1.paragraphs[0]
    ...
    tb_sub = s1.shapes.add_textbox(Inches(1.3), Inches(4.0), Inches(13.4), Inches(2.8))
    tf_sub = tb_sub.text_frame
    tf_sub.word_wrap = True

    cover_bullets = [
        "1. **技術長與架構師戰略對齊**：整合 ISO 42001 (AIMS)、CMMC Level 2 認證、Threat Hunting 與 Cyber Range AI 應用，劃分 RoE 交戰規則授權邊界",
        "2. **國家級總體藍圖與三維縱深防禦**：NCSIST AIEC 5大戰術柱石、無 GPS 邊緣硬體防篡改與緊急模型自毀 (<100ms)、聯邦學習「模型動，資料不動」、Data/Model Provenance 溯源",
        "3. **權威標準與量化指標全集**：精準融合 ISO 42001, NIST AI RMF 1.0, DoD CDAO T&E, MITRE ATLAS 與 15 項量化評測數學公式 (高解析資訊圖表採用 **Nano Banana pro** 繪製)"
    ]
    add_formatted_bullets(tf_sub, cover_bullets, font_size=14.5, text_color=RGBColor(226, 232, 240), bold_color=RGBColor(255, 255, 255))

    s1_acronyms = ["AIEC", "NCSIST", "NIST", "RMF", "DoD", "CDAO", "T&E", "ISO", "AIMS", "CMMC", "MITRE", "ATLAS", "MSR", "garak", "SOP"]
    ...
    c2_bullets = [
        ...
        "6. **第六區塊：國防 AI 安全進階防禦與主權架構 (P.30 - 37)**",
        "   - P.30 NCSIST 藍圖 | P.31 RoE 邊界 | P.32 邊緣自毀 | P.33 蒸餾 | P.34 展望",
        "   - P.35 評測流程圖 | P.36 3D防禦矩陣 | P.37 四層算力 (Nano Banana pro 資訊圖表)"
    ]
    add_formatted_bullets(tb_c2.text_frame, c2_bullets, font_size=11.5)
    ...
    s10_bullets = [
        ...
        "3. **審計機制 (Auditing)**：資料與模型溯源 (Data & Model Provenance 清洗規則、版本號)、決策可解釋性 (XAI 特徵熱力圖)、信心分數 (Confidence Score) 校準、高保真日誌包含輸入傳感器數據與行動方案 (COA)。"
    ]
    ...
    # ----------------------------------------------------
    # SECTION 6: Slides 30-37 - Advanced Defense AI Security & Infrastructure
    # ----------------------------------------------------
    ...
    tb_s32_r = s32.shapes.add_textbox(Inches(8.5), Inches(2.5), Inches(6.4), Inches(2.1))
    s32_r_bullets = [
        "1. **緊急權重複寫指令**：當系統偵測到物理入侵或接獲中斷信號時，極速觸發自毀回應時間：",
        "2. **Flash/RAM 雜訊衝刷**：以隨機雜訊迅速覆寫模型記憶體與快閃記憶體。",
    ...
    s32.shapes.add_picture(img_tau_abort, Inches(8.7), Inches(4.7), width=Inches(4.5))
    ...
    # Slide 34: CONCLUSION & FUTURE ROADMAP
    s34 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s34)
    add_header(s34, "結論與未來展望 (Conclusion & Future Roadmap)", "SECTION 6: ADVANCED DEFENSE AI SECURITY & INFRASTRUCTURE")

    add_icon_card(s34, 0.8, 1.5, 7.0, 5.9, "🏛️", "國防級 AI 評測結語", "Key Conclusions of Defense AI Evaluation", accent_color=BLUE)
    tb_s34_l = s34.shapes.add_textbox(Inches(1.1), Inches(2.5), Inches(6.4), Inches(4.7))
    s34_l_bullets = [
        "1. **制度與規範雙軌確立**：成功融合 ISO 42001 (AIMS) 與 NCSIST AIEC 總體藍圖，建構「通用方法論 + 戰術放行閘門」的雙支柱治理體系。",
        "2. **三維縱深防禦整合**：落實對抗防禦 (ATLAS)、無 GPS 邊緣防篡改與模型自毀 (回應時間極速)、聯邦學習保密與 Data/Model Provenance 完整溯源。",
        "3. **全面量化驗證體系**：透過 Q1~Q15 15 項量化指標與獨立 SOP，為國防 AI 系統提供可稽核、可重現且具備 TRL 合格品質的評測標準。"
    ]
    add_formatted_bullets(tb_s34_l.text_frame, s34_l_bullets, font_size=12.5)

    add_icon_card(s34, 8.2, 1.5, 7.0, 5.9, "🚀", "未來發展展望與藍圖", "Future Strategic Roadmap & MAITE Test Factory", accent_color=DARK_BLUE)
    tb_s34_r = s34.shapes.add_textbox(Inches(8.5), Inches(2.5), Inches(6.4), Inches(4.7))
    s34_r_bullets = [
        "1. **MAITE 自動化評測工具鏈整合**：推動與 MAITE (Model & AI Test Framework) 開放標準整合，對接 garak, RAGAS 與 AgentBench，實現 MLOps 自動化驗測工廠。",
        "2. **LVC 戰場實戰模擬擴充**：深化 VBS 4 / EADSIM 虛實整合平行戰場演練，將動態情境注入對抗攻擊擴充至空中蜂群與海面自主載具。",
        "3. **國防資安認證合規 (CMMC L2 / ISO 42001)**：推動 AIEC 確效紀錄自動轉換為 CMMC L2 控制項與 ISO 42001 審計日誌，加速武器系統 TRL 放行與營運部署。"
    ]
    add_formatted_bullets(tb_s34_r.text_frame, s34_r_bullets, font_size=12.5)
    add_acronym_footer(s34, ["AIEC", "NCSIST", "MAITE", "MLOps", "LVC", "VBS", "EADSIM", "CMMC", "ISO", "AIMS", "TRL", "SOP"], y_pos=7.60, height=1.0)

    # ----------------------------------------------------
    # Infographic Slide 1: Slide 35 (Nano Banana pro Badge)
    # ----------------------------------------------------
    s35 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s35)
    add_header(s35, "AIEC 國防 AI 評測與認證五大階段流程圖 (Infographic)", "SECTION 6: ADVANCED DEFENSE AI SECURITY & INFRASTRUCTURE")
    add_icon_card(s35, 0.8, 1.4, 14.4, 1.4, "🗺️", "國防 AIEC 評測五大階段架構流程圖 (Nano Banana pro)", "Five-Stage Evaluation Pipeline Flowchart", accent_color=BLUE)
    s35.shapes.add_picture(os.path.join('infographics', 'info_pipeline_s35.png'), Inches(0.8), Inches(2.9), width=Inches(14.4))

    tb_desc35 = s35.shapes.add_textbox(Inches(0.8), Inches(6.5), Inches(14.4), Inches(1.0))
    p35_desc = [
        "📌 **資訊圖表說明 (45 字 / Nano Banana pro 繪製)**：本圖展現 AIEC 國防 AI 評測 5 大階段標準流程，從需求與交戰規則 (RoE) 界定、數據清洗與 Provenance 溯源、Cyber Range 對抗演練，到 15 項量化評測與 TRL 戰術部署放行。"
    ]
    add_formatted_bullets(tb_desc35.text_frame, p35_desc, font_size=12.0)
    add_acronym_footer(s35, ["AIEC", "RoE", "TRL", "CMMC", "SOP"], y_pos=7.60, height=1.0)

    # ----------------------------------------------------
    # Infographic Slide 2: Slide 36 (Nano Banana pro Badge)
    # ----------------------------------------------------
    s36 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s36)
    add_header(s36, "國防 AI 安全保密審計 3D 縱深防禦矩陣 (Infographic)", "SECTION 6: ADVANCED DEFENSE AI SECURITY & INFRASTRUCTURE")
    add_icon_card(s36, 0.8, 1.4, 14.4, 1.4, "🛡️", "國防 AI 安全保密審計 3D 縱深防禦矩陣 (Nano Banana pro)", "3D Defense Matrix (Security, Confidentiality & Auditing)", accent_color=DARK_BLUE)
    s36.shapes.add_picture(os.path.join('infographics', 'info_3d_matrix_s36.png'), Inches(0.8), Inches(2.9), width=Inches(14.4))

    tb_desc36 = s36.shapes.add_textbox(Inches(0.8), Inches(6.5), Inches(14.4), Inches(1.0))
    p36_desc = [
        "📌 **資訊圖表說明 (46 字 / Nano Banana pro 繪製)**：本圖呈現國防 AI 三維縱深防禦矩陣。整合 Security (ATLAS 對抗防禦與硬體自毀)、Confidentiality (地端實體隔離與聯邦學習) 與 Auditing (Data/Model Provenance 溯源與 XAI 特徵熱力圖)。"
    ]
    add_formatted_bullets(tb_desc36.text_frame, p36_desc, font_size=12.0)
    add_acronym_footer(s36, ["ATLAS", "GPS", "SPIFFE", "OPA", "LoRA", "GGUF", "ECE", "COA"], y_pos=7.60, height=1.0)

    # ----------------------------------------------------
    # Infographic Slide 3: Slide 37 (Nano Banana pro Badge)
    # ----------------------------------------------------
    s37 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s37)
    add_header(s37, "主權 AI 四層算力堆疊與戰術邊緣部署 (Infographic)", "SECTION 6: ADVANCED DEFENSE AI SECURITY & INFRASTRUCTURE")
    add_icon_card(s37, 0.8, 1.4, 14.4, 1.4, "🏛️", "主權 AI 四層算力堆疊與邊緣部署架構圖 (Nano Banana pro)", "Sovereign AI Four-Tier Compute Stack Pyramid", accent_color=BLUE)
    s37.shapes.add_picture(os.path.join('infographics', 'info_4tier_compute_s37.png'), Inches(0.8), Inches(2.9), width=Inches(14.4))

    tb_desc37 = s37.shapes.add_textbox(Inches(0.8), Inches(6.5), Inches(14.4), Inches(1.0))
    p37_desc = [
        "📌 **資訊圖表說明 (44 字 / Nano Banana pro 繪製)**：本圖展現民雄院區主權 AI 四層算力架構。由 Tier 1 密集型 70B 主權模型至 Tier 3 邊緣 7B/3B 蒸餾量化模型，搭配邊緣硬體自毀機制，達成極致算力與戰術保密。"
    ]
    add_formatted_bullets(tb_desc37.text_frame, p37_desc, font_size=12.0)
    add_acronym_footer(s37, ["MoE", "GGUF", "LoRA", "SOP"], y_pos=7.60, height=1.0)

    out_dir = r'c:\Users\administartor\Downloads\AIEC'
    out_path = os.path.join(out_dir, 'AIEC_AI_Evaluation_30_Slides_NanoBanana.pptx')
    prs.save(out_path)
    print(f'Successfully generated master 37-slide presentation at: {out_path}')

if __name__ == '__main__':
    generate_37_slides_master_deck()
