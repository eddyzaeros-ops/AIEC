...
def add_infographic_card(slide, x, y, width, height, img_filename, label_zh, label_en="Nano Banana Pro Infographic"):
    NAVY = RGBColor(12, 35, 64)
...
def build_30_ai_eval_deck():
    ...
    cover_bullets = [
        ...
        "4. **Nano Banana Pro 視效**：整合高解析度 3D 視覺化 AI 評測資訊圖表 (Infographics)"
    ]
    ...
    build_2card_slide_fn(
        "AI 評測雙腦知識庫架構與 MOC 主索引頁", "SECTION 1: EVALUATION FRAMEWORK & GOVERNANCE",
        "🧠", "1. Karpathy 評測知識流結構", "3-Tier Evaluation Vault",
        [
            "1. **輸入層 (Clippings/)**：自動剪藏評測規範、論文與攻防數據",
            "2. **消化層 (知識庫/)**：AI 自動提煉評測指標、維護 SOP 索引",
            "3. **輸出層 (創作庫/)**：生成專業 T&E 評測報告與合規稽核表"
        ],
        "🗂️", "2. AIEC 評測專題目錄配置", "Evaluation Subfolder Hierarchy",
        [
            "00-Index_and_Templates/：**MOC 主索引與 3 大評測筆記範本**",
            "01-治理與規範/：**ISO 42001, SHIELD, ATLAS, RAG 權限稽核**",
            "02-評測矩陣與構面/：**15 項指標 SOP, 4 大能力層次, JATIC 7 構面**",
            "03-應用系統評測/：**A~F 六類 CV/LLM/RAG/Agent/HMT 評測 SOP**",
            "04-地端架構與邊緣算力/：**主權 LLM 測試堆疊, Ollama/vLLM Benchmark**"
        ],
    ...
    # ----------------------------------------------------
    # Slide 11: System A - Computer Vision (INFOGRAPHIC SLIDE 1)
    # ----------------------------------------------------
    build_infographic_slide(
        ...
        "adversarial_attack_defense.jpg", "Nano Banana Pro: 對抗干擾與視覺防禦評測資訊圖表",
    ...
    build_2card_slide_fn(
        ...
        [
            ...
            "3. **敏感資料洩露測試**：評測機密脈絡遭 Prompt Inversion 導出風險"
        ],
    ...
    # ----------------------------------------------------
    # Slide 13: System C - RAG (INFOGRAPHIC SLIDE 2)
    # ----------------------------------------------------
    build_infographic_slide(
        ...
        [
            ...
            "3. **RAG Triad 評測三元組**：脈絡精確度 (Precision)、忠實度 (Faithfulness) 與相關度",
            "4. **代表工具鏈**：RAGAS Assessment, TruLens Framework, Arize Phoenix"
        ],
        "rag_triad_eval.jpg", "Nano Banana Pro: RAG 檢索增強生成三元組評測架構圖",
        ["RAG", "LLM", "RAGAS", "SOP"]
    )
    ...
    # ----------------------------------------------------
    # Slide 15: System E - Autonomous Systems & HMT (INFOGRAPHIC SLIDE 3)
    # ----------------------------------------------------
    build_infographic_slide(
        ...
        [
            ...
            "3. **認知負荷測試 (NASA-TLX)**：評測介面警報過多引發之決策恐慌",
            "4. **評測規範與工具**：DoDD 3000.09 自主武器指令, ToAST, IDA HMT Guidebook"
        ],
        "hmt_trust_calibration.jpg", "Nano Banana Pro: HMT 人機協同信任校準與認知負荷評測儀表板",
    ...
    # ----------------------------------------------------
    # Slide 17: 15 Metrics Master Overview (INFOGRAPHIC SLIDE 4)
    # ----------------------------------------------------
    build_infographic_slide(
        ...
        "ai_eval_matrix.jpg", "Nano Banana Pro: 15 項 AI 量化評測指標與三大視角矩陣圖",
    ...
    # ----------------------------------------------------
    # Slide 26: Sovereign AI Platform (INFOGRAPHIC SLIDE 5)
    # ----------------------------------------------------
    build_infographic_slide(
        ...
        "sovereign_ai_test_stack.jpg", "Nano Banana Pro: 4-Tier 主權 LLM 白箱 vs 黑箱評測架構圖",
    ...
    prs.save(out_path)
    print(f'Successfully generated 30-slide AI Evaluation presentation with Nano Banana Pro infographics at: {out_path}')
