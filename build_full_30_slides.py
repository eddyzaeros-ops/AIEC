...
def build_30_deck():
    ...
    # ----------------------------------------------------
    # Slide 1: Cover (Pure White Theme with Navy Accent Container)
    # ----------------------------------------------------
    s1 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s1)

    # Main Cover Card
    cover_card = s1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(0.8), Inches(14.4), Inches(7.4))
    cover_card.fill.solid()
    cover_card.fill.fore_color.rgb = NAVY
    cover_card.line.fill.background()

    # Badge Pill
    badge = s1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(1.3), Inches(1.5), Inches(5.6), Inches(0.55))
    badge.fill.solid()
    ...
    # Title Box
    tb1 = s1.shapes.add_textbox(Inches(1.3), Inches(2.4), Inches(13.4), Inches(2.2))
    tf1 = tb1.text_frame
    tf1.word_wrap = True
    p = tf1.paragraphs[0]
    ...
    # Subtitle Bullets
    tb_sub = s1.shapes.add_textbox(Inches(1.3), Inches(4.8), Inches(13.4), Inches(2.8))
    ...
    # Helper function for standard 2-card slides
    def build_2card_slide(title, category, icon1, t1_zh, t1_en, b1, icon2, t2_zh, t2_en, b2, accent1=BLUE, accent2=DARK_BLUE):
        s = prs.slides.add_slide(blank_layout)
        set_pure_white_bg(s)
        add_header(s, title, category)

        # Card 1 (Left)
        add_icon_card(s, 0.8, 1.6, 7.0, 6.8, icon1, t1_zh, t1_en, accent_color=accent1)
        tb_b1 = s.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(6.4), Inches(5.4))
        add_formatted_bullets(tb_b1.text_frame, b1, font_size=14)

        # Card 2 (Right)
        add_icon_card(s, 8.2, 1.6, 7.0, 6.8, icon2, t2_zh, t2_en, accent_color=accent2)
        tb_b2 = s.shapes.add_textbox(Inches(8.5), Inches(2.7), Inches(6.4), Inches(5.4))
        add_formatted_bullets(tb_b2.text_frame, b2, font_size=14)
        return s

    # ----------------------------------------------------
    # Slide 2: Knowledge Vault Structure & MOC
    # ----------------------------------------------------
    build_2card_slide(
    ...
    # Helper function for System T&E slides (A~F)
    def build_system_slide(title, category, sys_code, sys_name, icon, risks, sop_steps, tools):
        s = prs.slides.add_slide(blank_layout)
        set_pure_white_bg(s)
        add_header(s, title, category)

        # Left Card: Risks & Scope
        add_icon_card(s, 0.8, 1.6, 7.0, 6.8, icon, f"{sys_code} - {sys_name}", "System Risk Profile & Scope", accent_color=BLUE)
        tb_r = s.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(6.4), Inches(5.4))
        add_formatted_bullets(tb_r.text_frame, risks, font_size=14)

        # Right Card: SOP & Tools
        add_icon_card(s, 8.2, 1.6, 7.0, 6.8, "🛠️", "評測 SOP 與工具鏈", "Testing Methodology & Toolkits", accent_color=DARK_BLUE)
        tb_s = s.shapes.add_textbox(Inches(8.5), Inches(2.7), Inches(6.4), Inches(5.4))
        items = ["**測試實施 SOP**："] + ["  " + step for step in sop_steps] + ["**代表性工具鏈**："] + ["  " + tool for tool in tools]
        add_formatted_bullets(tb_s.text_frame, items, font_size=14)
    ...
    # Helper function for 15 Metrics pairs (Slides 18-24)
    def build_metric_pair_slide(title, category, m1, m2):
        s = prs.slides.add_slide(blank_layout)
        set_pure_white_bg(s)
        add_header(s, title, category)
        ...
        for idx, m in enumerate(metrics):
            ...
            # Card Base
            card = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(x), Inches(1.6), Inches(7.0), Inches(6.8))
            card.fill.solid()
            ...
            # Content Bullets
            tb_c = s.shapes.add_textbox(Inches(x + 0.3), Inches(2.9), Inches(6.4), Inches(5.3))
            ...
            add_formatted_bullets(tf_c, bullet_items, font_size=13)
    ...
    build_metric_pair_slide("15 項評測指標 (Q1 - Q2) —— 對抗與自然穩健性", "SECTION 4: 15 QUANTITATIVE METRICS & SOPS", m1, m2)
    ...
    build_metric_pair_slide("15 項評測指標 (Q3 - Q4) —— 任務完成與失效安全", "SECTION 4: 15 QUANTITATIVE METRICS & SOPS", m3, m4)
    ...
    build_metric_pair_slide("15 項評測指標 (Q5 - Q6) —— 信任校準與認知負荷", "SECTION 4: 15 QUANTITATIVE METRICS & SOPS", m5, m6)
    ...
    build_metric_pair_slide("15 項評測指標 (Q7 - Q8) —— 可解釋性與越獄防禦", "SECTION 4: 15 QUANTITATIVE METRICS & SOPS", m7, m8)
    ...
    build_metric_pair_slide("15 項評測指標 (Q9 - Q10) —— 幻覺控制與 RAG 精確度", "SECTION 4: 15 QUANTITATIVE METRICS & SOPS", m9, m10)
    ...
    build_metric_pair_slide("15 項評測指標 (Q11 - Q12) —— Agent 合規與漂移監控", "SECTION 4: 15 QUANTITATIVE METRICS & SOPS", m11, m12)
    ...
    build_metric_pair_slide("15 項評測指標 (Q13 - Q14) —— 不確定性與防降密洩漏", "SECTION 4: 15 QUANTITATIVE METRICS & SOPS", m13, m14)
    ...
    # Create single metric focus for Q15
    ...
    add_icon_card(s25, 0.8, 1.6, 14.4, 6.8, "📜", "Q15 - 系統軌跡可追溯性與可稽核性 (Traceability & Audit Compliance)", "CMMC Level 2 & ISO 42001 Full Auditability", accent_color=DARK_BLUE)
    tb_q15 = s25.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(5.4))
    ...
    out_path = os.path.join(out_dir, 'AIEC_Full_30_Slides_Presentation.pptx')
    prs.save(out_path)
    print(f'Successfully generated 30-slide presentation at: {out_path}')
