...
def add_native_toc_field(doc, instruction='TOC \\o "1-3" \\h \\z \\u'):
    p = doc.add_paragraph(style='Normal')
    p.paragraph_format.space_before = Pt(4)
    p.paragraph_format.space_after = Pt(12)
    instr_xml = instruction.replace('"', '&quot;')
    fldSimple = parse_xml(f'<w:fldSimple {nsdecls("w")} w:instr="{instr_xml}"/>')
...
def build_native_styled_docx():
    ...
    set_style_font_and_color(doc, 'Caption', Pt(9.5), bold=True, color_rgb=RGBColor(37, 99, 235))
    ...
    # ----------------------------------------------------
    # AUTOMATED TOC & LIST OF FIGURES & TABLES
    # ----------------------------------------------------
    add_heading_styled(doc, "📋 目錄 (Table of Contents)", level=1)
    
    # Word Auto-Generated TOC Field
    add_native_toc_field(doc, instruction='TOC \\o "1-3" \\h \\z \\u')

    # Pre-rendered TOC lines with Page Numbers for immediate preview
    toc_items = [
    ...
    # Word Auto-Generated Table of Figures Field
    add_native_toc_field(doc, instruction='TOC \\h \\z \\t "Figure,圖,Caption"')
    ...
    # Word Auto-Generated Table of Tables Field
    add_native_toc_field(doc, instruction='TOC \\h \\z \\t "Table,表"')
    ...
    p_t1 = doc.add_paragraph(style='Caption')
    p_t1.paragraph_format.space_before = Pt(6)
    p_t1.paragraph_format.space_after = Pt(4)
    r_t1 = p_t1.add_run("表 1-1：傳統軍事戰術 vs. 非對稱 AI 戰術比較表")
    set_run_fonts(r_t1, font_size=Pt(10.5), bold=True, color_rgb=RGBColor(12, 35, 64))
    ...
    # Table 2-1: ISO vs AIEC
    p_t21 = doc.add_paragraph(style='Caption')
    p_t21.paragraph_format.space_before = Pt(6)
    p_t21.paragraph_format.space_after = Pt(4)
    r_t21 = p_t21.add_run("表 2-1：ISO 42001 (AIMS) 通用標準 vs. AIEC 國防權責分工比較表")
    set_run_fonts(r_t21, font_size=Pt(10.5), bold=True, color_rgb=RGBColor(12, 35, 64))
    ...
    # Table 2-2: RoE Comparison
    p_t22 = doc.add_paragraph(style='Caption')
    p_t22.paragraph_format.space_before = Pt(8)
    p_t22.paragraph_format.space_after = Pt(4)
    r_t22 = p_t22.add_run("表 2-2：交戰規則 (RoE) 人機授權三階權能邊界比較表 (HITL / HOTL / HOOTL)")
    set_run_fonts(r_t22, font_size=Pt(10.5), bold=True, color_rgb=RGBColor(12, 35, 64))
    ...
    # Embed Figure 2-1: Pipeline Flowchart
    p_fig21 = doc.add_paragraph(style='Caption')
    ...
    p_fig21_cap = doc.add_paragraph(style='Caption')
    p_fig21_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_fig21_cap.paragraph_format.space_after = Pt(12)
    r_cap21 = p_fig21_cap.add_run("圖 2-1：AIEC 國防 AI 評測五大階段標準流程圖 (Nano Banana pro 生成)")
    set_run_fonts(r_cap21, font_size=Pt(9.5), bold=True, color_rgb=RGBColor(37, 99, 235))
    ...
    # Table 2-3: 15 Quantitative Metrics
    p_t23 = doc.add_paragraph(style='Caption')
    p_t23.paragraph_format.space_before = Pt(6)
    p_t23.paragraph_format.space_after = Pt(4)
    r_t23 = p_t23.add_run("表 2-3：AIEC 15 項量化評測指標 (Q1~Q15) 與合格門檻對照總表")
    set_run_fonts(r_t23, font_size=Pt(10.5), bold=True, color_rgb=RGBColor(12, 35, 64))
    ...
    # Embed Figure 2-2: 3D Defense Matrix
    p_fig22 = doc.add_paragraph(style='Caption')
    ...
    p_fig22_cap = doc.add_paragraph(style='Caption')
    p_fig22_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_fig22_cap.paragraph_format.space_after = Pt(12)
    r_cap22 = p_fig22_cap.add_run("圖 2-2：國防 AI 安全保密審計 3D 縱深防禦矩陣圖 (Nano Banana pro 生成)")
    set_run_fonts(r_cap22, font_size=Pt(9.5), bold=True, color_rgb=RGBColor(37, 99, 235))
    ...
    # Embed Figure 2-3: 4-Tier Compute Stack
    p_fig23 = doc.add_paragraph(style='Caption')
    ...
    p_fig23_cap = doc.add_paragraph(style='Caption')
    p_fig23_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_fig23_cap.paragraph_format.space_after = Pt(12)
    r_cap23 = p_fig23_cap.add_run("圖 2-3：民雄院區國家級主權 AI 四層算力堆疊架構圖 (Nano Banana pro 生成)")
    set_run_fonts(r_cap23, font_size=Pt(9.5), bold=True, color_rgb=RGBColor(37, 99, 235))
    ...
    p_t31 = doc.add_paragraph(style='Caption')
    p_t31.paragraph_format.space_before = Pt(6)
    p_t31.paragraph_format.space_after = Pt(4)
    r_t31 = p_t31.add_run("表 3-1：AIEC 國防 AI 導入專案 24 個月執行時程規劃表")
    set_run_fonts(r_t31, font_size=Pt(10.5), bold=True, color_rgb=RGBColor(12, 35, 64))
    ...
    # Table 4-1: Resources
    p_t41 = doc.add_paragraph(style='Caption')
    p_t41.paragraph_format.space_before = Pt(6)
    p_t41.paragraph_format.space_after = Pt(4)
    r_t41 = p_t41.add_run("表 4-1：專案人力、主權算力與軟硬體資源配置表")
    set_run_fonts(r_t41, font_size=Pt(10.5), bold=True, color_rgb=RGBColor(12, 35, 64))
    ...
    p_t42 = doc.add_paragraph(style='Caption')
    p_t42.paragraph_format.space_before = Pt(6)
    p_t42.paragraph_format.space_after = Pt(4)
    r_t42 = p_t42.add_run("表 4-2：國防 AI 導入主要風險評估與處置矩陣表")
    set_run_fonts(r_t42, font_size=Pt(10.5), bold=True, color_rgb=RGBColor(12, 35, 64))
    ...
    print(f"Successfully generated Word proposal document with native styles, TOC fields & page numbers at:\n - {out_local}\n - {out_desktop}")

if __name__ == '__main__':
    build_native_styled_docx()
