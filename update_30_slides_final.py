...
def generate_30_slides_master_deck():
    prs = Presentation()
    ...
    badge = s1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(1.3), Inches(1.1), Inches(7.2), Inches(0.55))
    badge.fill.solid()
    badge.fill.fore_color.rgb = BLUE
    badge.line.fill.background()
    p = badge.text_frame.paragraphs[0]
    p.text = "🛡️ AIEC 國防與企業級 AI 專屬評測與驗測 SOP 體系"
    ...
    tf1.word_wrap = True
    p = tf1.paragraphs[0]
    p.text = "AIEC 國防 AI 評測與認證體系 Master SOP"
    p.font.size = Pt(36)
    p.font.bold = True
    p.font.color.rgb = RGBColor(255, 255, 255)
    p.font.name = "微軟正黑體"
    ...
    s1_acronyms = ["NCSIST", "AIEC", "NIST", "RMF", "DoD", "CDAO", "T&E", "ISO", "AIMS", "MITRE", "ATLAS", "SOP"]
    ...
    add_header(s2, "TABLE OF CONTENTS", "AIEC GOVERNANCE & EVALUATION MASTER DECK")

    add_icon_card(s2, 0.8, 1.6, 7.0, 6.25, "📋", "簡報導覽與核心區塊 (1-3)", "Governance, Methodology & System SOPs", accent_color=BLUE)
    tb_c1 = s2.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(6.4), Inches(4.8))
    c1_bullets = [
        "1. **第一區塊：評測框架與雙支柱治理 (P.3 - P.6)**",
        "   - NCSIST AIEC 藍圖與 5 大底層戰術柱石",
        "   - SHIELD 治理循環與 ISO 42001 (AIMS) 方法論",
        "   - MITRE ATLAS 16 大戰術與 ATT&CK 對比",
        "2. **第二區塊：T&E 評測矩陣與方法論 (P.7 - P.10)**",
        "   - DoD CDAO Level 1~4 四大能力層次",
        "   - JATIC 七大共通構面與 6 大評測方法論",
        "   - 安全、保密與審計三維縱深防禦矩陣",
        "3. **第三區塊：A~F 類應用系統評測 SOP (P.11 - P.16)**",
        "   - 電腦視覺 (CV)、LLM/GenAI、RAG 知識庫",
        "   - AI Agent 軌跡、自主系統 HMT (RoE) 與決策支援"
    ]
    add_formatted_bullets(tb_c1.text_frame, c1_bullets, font_size=13.5)

    add_icon_card(s2, 8.2, 1.6, 7.0, 6.25, "📐", "簡報導覽與核心區塊 (4-5)", "Quantitative Metrics & Platform Architecture", accent_color=DARK_BLUE)
    tb_c2 = s2.shapes.add_textbox(Inches(8.5), Inches(2.7), Inches(6.4), Inches(4.8))
    c2_bullets = [
        "4. **第四區塊：15 項量化評測指標與 SOP (P.17 - P.25)**",
        "   - Q1~Q15 完整量化計算公式 (300 DPI LaTeX 渲染)",
        "   - Pass/Fail 合格判定門檻與代表性測試工具鏈",
        "   - 對抗韌性、MSR、RAGAS、garak、PyOD 評測細節",
        "5. **第五區塊：評測平台、主權算力與地端工具 (P.26 - P.30)**",
        "   - 主權 AI 平台與民雄院區四層 LLM 算力堆疊",
        "   - 地端模型蒸餾 (Knowledge Distillation) 與 GGUF/LoRA",
        "   - Lattice C2 / Menace 零信任 API 閘道對接",
        "   - 邊緣硬體防篡改與緊急模型自毀 (<100ms)",
        "   - 聯邦學習 (Federated Learning) 參數融合在地"
    ]
    add_formatted_bullets(tb_c2.text_frame, c2_bullets, font_size=13.5)
    add_acronym_footer(s2, ["AIEC", "NCSIST", "SHIELD", "ISO", "AIMS", "ATLAS", "JATIC", "RAGAS", "garak", "PyOD"], y_pos=7.95, height=0.9)
    ...
    add_icon_card(s3, 0.8, 1.6, 7.0, 6.25, "🏛️", "支柱一：NCSIST AIEC 藍圖與 DAGR 風險指南", "NCSIST AIEC Blueprint & DAGR Guidelines", accent_color=BLUE)
    tb_s3_l = s3.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(6.4), Inches(4.8))
    s3_l_bullets = [
    ...
    add_formatted_bullets(tb_s3_l.text_frame, s3_l_bullets, font_size=13.0)

    add_icon_card(s3, 8.2, 1.6, 7.0, 6.25, "🔄", "支柱二：SHIELD 六大治理循環", "SHIELD 6-Stage Governance Lifecycle", accent_color=DARK_BLUE)
    tb_s3_r = s3.shapes.add_textbox(Inches(8.5), Inches(2.7), Inches(6.4), Inches(4.8))
    s3_r_bullets = [
    ...
    add_formatted_bullets(tb_s3_r.text_frame, s3_r_bullets, font_size=13.0)
    add_acronym_footer(s3, ["NCSIST", "AIEC", "DAGR", "SHIELD", "CMMC", "RoE", "HITL", "HOTL", "HOOTL", "TRL", "COA"], y_pos=7.95, height=0.9)
    ...
    add_icon_card(s4, 0.8, 1.6, 14.4, 6.25, "🔄", "SHIELD 六大治理階段與審計控制點", "Six Governance Stages & Audit Control Points", accent_color=BLUE)
    tb_s4 = s4.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s4_bullets = [
        ...
        "6. **Detect (漂移監控)**：營運期間部署 PyOD 與 Alibi 監控模組，實現 $t_{\mathrm{alarm\_latency}} \le 5\mathrm{min}$ 之即時告警。"
    ]
    add_formatted_bullets(tb_s4.text_frame, s4_bullets, font_size=14.0)
    add_acronym_footer(s4, ["SHIELD", "CMMC", "RoE", "HITL", "HOTL", "HOOTL", "TRL", "COA", "PyOD", "AIEC"], y_pos=7.95, height=0.9)
    ...
    add_icon_card(s5, 0.8, 1.6, 7.0, 6.25, "🌐", "ISO 42001：通用框架與方法論", "Framework & Methodology (AIMS)", accent_color=BLUE)
    tb_s5_l = s5.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(6.4), Inches(4.8))
    s5_l_bullets = [
    ...
    add_formatted_bullets(tb_s5_l.text_frame, s5_l_bullets, font_size=13.5)

    add_icon_card(s5, 8.2, 1.6, 7.0, 6.25, "🎯", "AIEC：領域化審查閘門與規則制定者", "Domain-Specific Gatekeeper & TRL Enforcement", accent_color=DARK_BLUE)
    tb_s5_r = s5.shapes.add_textbox(Inches(8.5), Inches(2.7), Inches(6.4), Inches(4.8))
    s5_r_bullets = [
    ...
    add_formatted_bullets(tb_s5_r.text_frame, s5_r_bullets, font_size=13.5)
    add_acronym_footer(s5, ["ISO", "AIMS", "AIIA", "AIEC", "TRL", "RoE", "HITL", "HOTL", "HOOTL", "LVC", "VBS", "EADSIM", "RAG"], y_pos=7.95, height=0.9)
    ...
    add_icon_card(s6, 0.8, 1.6, 7.0, 6.25, "⚔️", "ATLAS 威脅矩陣與 16 大戰術鏈", "ATLAS Matrix & 16 Tactics Chain", accent_color=BLUE)
    tb_s6_l = s6.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(6.4), Inches(4.8))
    s6_l_bullets = [
    ...
    add_formatted_bullets(tb_s6_l.text_frame, s6_l_bullets, font_size=13.0)

    add_icon_card(s6, 8.2, 1.6, 7.0, 6.25, "📊", "ATT&CK vs. ATLAS 比較與紅軍演練", "ATT&CK vs. ATLAS Comparison & Red Teaming", accent_color=DARK_BLUE)
    tb_s6_r = s6.shapes.add_textbox(Inches(8.5), Inches(2.7), Inches(6.4), Inches(4.8))
    s6_r_bullets = [
    ...
    add_formatted_bullets(tb_s6_r.text_frame, s6_r_bullets, font_size=13.0)
    add_acronym_footer(s6, ["MITRE", "ATLAS", "ATT&CK", "OWASP", "LLM", "API", "garak", "ART", "AIEC"], y_pos=7.95, height=0.9)

    # ----------------------------------------------------
    # SECTION 2: Slide 7 - T&E 四大能力層次
    # ----------------------------------------------------
    s7 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s7)
    add_header(s7, "DoD CDAO T&E 四大能力層次", "SECTION 2: T&E MATRIX & METHODOLOGY")

    add_icon_card(s7, 0.8, 1.6, 14.4, 6.25, "📐", "DoD CDAO AI T&E 漸進能力評測階梯", "Progressive T&E Capability Hierarchy", accent_color=BLUE)
    tb_s7 = s7.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s7_bullets = [
        "1. **Level 1: 基礎模型評測 (Base Model T&E)**：驗證演算法精確度、對抗韌性 ($\mathrm{Acc}_{\mathrm{adv}}/\mathrm{Acc}_{\mathrm{clean}} \ge 0.90$) 與自然穩健性 ($\Delta \mathrm{mAP} \le 0.10$)。",
        "2. **Level 2: 人機系統整合 (HSI T&E)**：評估指揮官與操作員之認知負荷 ($\Delta \mathrm{TLX} \ge 0.30$)、眼動追蹤與信任校準 ($\mathrm{ECE} \le 0.05$)。",
        "3. **Level 3: 系統整合評測 (Systems Integration T&E)**：驗證 API 閘道零信任 (SPIFFE/OPA)、工具調用邊界 ($R_{\mathrm{unauth\_API}} = 0\%$) 與通訊時延。",
        "4. **Level 4: 作戰性測試與評估 (Operational T&E)**：於 VBS 4 / EADSIM LVC 虛實整合平行戰場中，確效端到端任務完成率 ($\mathrm{MSR} \ge 0.95$) 與擊殺鏈閉合速度。"
    ]
    add_formatted_bullets(tb_s7.text_frame, s7_bullets, font_size=14.0)
    add_acronym_footer(s7, ["DoD", "CDAO", "T&E", "HSI", "NASA-TLX", "ECE", "SPIFFE", "OPA", "API", "VBS", "EADSIM", "LVC", "MSR"], y_pos=7.95, height=0.9)

    # ----------------------------------------------------
    # SECTION 2: Slide 8 - JATIC 七大共通構面
    # ----------------------------------------------------
    s8 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s8)
    add_header(s8, "JATIC 七大共通構面 (JATIC Common Dimensions)", "SECTION 2: T&E MATRIX & METHODOLOGY")

    add_icon_card(s8, 0.8, 1.6, 7.0, 6.25, "🏛️", "JATIC 七大構面 (1 - 4)", "Robustness, Resiliency, Explainability & Competence", accent_color=BLUE)
    tb_s8_l = s8.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(6.4), Inches(4.8))
    s8_l_bullets = [
    ...
    add_formatted_bullets(tb_s8_l.text_frame, s8_l_bullets, font_size=13.5)

    add_icon_card(s8, 8.2, 1.6, 7.0, 6.25, "⚖️", "JATIC 七大構面 (5 - 7)", "Fairness, Calibration & Drift Detection", accent_color=DARK_BLUE)
    tb_s8_r = s8.shapes.add_textbox(Inches(8.5), Inches(2.7), Inches(6.4), Inches(4.8))
    s8_r_bullets = [
    ...
    add_formatted_bullets(tb_s8_r.text_frame, s8_r_bullets, font_size=13.5)
    add_acronym_footer(s8, ["JATIC", "MSR", "ECE", "PyOD", "AIF360"], y_pos=7.95, height=0.9)

    # ----------------------------------------------------
    # SECTION 2: Slide 9 - 國防 AI 評測 6 大方法論
    # ----------------------------------------------------
    s9 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s9)
    add_header(s9, "國防 AI 評測 6 大方法論與 SOP", "SECTION 2: T&E MATRIX & METHODOLOGY")

    add_icon_card(s9, 0.8, 1.6, 14.4, 6.25, "🧪", "六大動態評測方法論全景圖", "Six Dynamic Evaluation Methodologies", accent_color=BLUE)
    tb_s9 = s9.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s9_bullets = [
    ...
    add_formatted_bullets(tb_s9.text_frame, s9_bullets, font_size=14.0)
    add_acronym_footer(s9, ["SHAP", "LIME", "garak", "NASA-TLX", "EEG", "HMT", "PyOD", "API"], y_pos=7.95, height=0.9)

    # ----------------------------------------------------
    # SECTION 2: Slide 10 - 國防 AI 安全保密與審計三維矩陣
    # ----------------------------------------------------
    s10 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s10)
    add_header(s10, "國防 AI 安全、保密與審計三維矩陣", "SECTION 2: T&E MATRIX & METHODOLOGY")

    add_icon_card(s10, 0.8, 1.6, 14.4, 6.25, "🛡️", "安全 (Security)、保密 (Confidentiality) 與審計 (Auditing) 聯防體系", "3D Defense Matrix", accent_color=BLUE)
    tb_s10 = s10.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s10_bullets = [
    ...
    add_formatted_bullets(tb_s10.text_frame, s10_bullets, font_size=14.0)
    add_acronym_footer(s10, ["ATLAS", "SPIFFE", "OPA", "API", "JADC2", "GGUF", "LoRA", "XAI", "COA"], y_pos=7.95, height=0.9)

    # ----------------------------------------------------
    # SECTION 3: System SOP Slides 11-16
    # ----------------------------------------------------
    # Slide 11: A類 - 電腦視覺
    s11 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s11)
    add_header(s11, "A類 - 電腦視覺與目標偵測評測 SOP", "SECTION 3: SYSTEM-SPECIFIC AI EVALUATION SOP")
    add_icon_card(s11, 0.8, 1.6, 14.4, 6.25, "👁️", "CV / Target Detection Evaluation SOP", accent_color=BLUE)
    tb_s11 = s11.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s11_bullets = [
        ...
        "3. **關鍵指標與合格門檻**：",
        "   - **Q1 對抗韌性**：$\mathrm{Robustness~Ratio} = \mathrm{Acc}_{\mathrm{adv}}/\mathrm{Acc}_{\mathrm{clean}} \ge 0.90 \quad (\epsilon \le 0.05)$",
        "   - **Q2 自然穩健性**：$\Delta \mathrm{mAP} \le 0.10 \quad (10\%\text{ Limit})$",
        "   - **Q7 模型可解釋性**：$\mathrm{Point~Game~Score} \ge 0.85$ (XAITK / SHAP 歸因熱力圖)"
    ]
    add_formatted_bullets(tb_s11.text_frame, s11_bullets, font_size=13.5)
    add_acronym_footer(s11, ["CV", "YOLO", "ISR", "FGSM", "PGD", "ART", "HEART", "NRTK", "mAP", "XAITK", "SHAP"], y_pos=7.95, height=0.9)

    # Slide 12: B類 - 生成式 AI 與 LLM
    s12 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s12)
    add_header(s12, "B類 - 生成式 AI 與大語言模型評測 SOP", "SECTION 3: SYSTEM-SPECIFIC AI EVALUATION SOP")
    add_icon_card(s12, 0.8, 1.6, 14.4, 6.25, "💬", "GenAI / LLM Evaluation SOP", accent_color=DARK_BLUE)
    tb_s12 = s12.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s12_bullets = [
        ...
        "3. **關鍵指標與合格門檻**：",
        "   - **Q8 提示越獄與抗注入**：$R_{\mathrm{jailbreak\_def}} = 1 - (N_{\mathrm{succ}}/N_{\mathrm{total}}) \ge 0.99 \quad (99\%)$",
        "   - **Q9 幻覺率與事實忠實度**：$\mathrm{Faithfulness} \ge 0.95 \quad \wedge \quad R_{\mathrm{hallucination}} \le 0.02$"
    ]
    add_formatted_bullets(tb_s12.text_frame, s12_bullets, font_size=13.5)
    add_acronym_footer(s12, ["GenAI", "LLM", "garak", "NeMo", "OWASP"], y_pos=7.95, height=0.9)

    # Slide 13: C類 - RAG 系統
    s13 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s13)
    add_header(s13, "C類 - 檢索增強生成 RAG 系統評測 SOP", "SECTION 3: SYSTEM-SPECIFIC AI EVALUATION SOP")
    add_icon_card(s13, 0.8, 1.6, 14.4, 6.25, "📚", "RAG Systems Evaluation SOP", accent_color=BLUE)
    tb_s13 = s13.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s13_bullets = [
    ...
    add_formatted_bullets(tb_s13.text_frame, s13_bullets, font_size=13.5)
    add_acronym_footer(s13, ["RAG", "RAGAS", "RBAC"], y_pos=7.95, height=0.9)

    # Slide 14: D類 - AI Agent
    s14 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s14)
    add_header(s14, "D類 - AI Agent 與多代理協同系統評測 SOP", "SECTION 3: SYSTEM-SPECIFIC AI EVALUATION SOP")
    add_icon_card(s14, 0.8, 1.6, 14.4, 6.25, "🤖", "AI Agent & Multi-Agent Evaluation SOP", accent_color=DARK_BLUE)
    tb_s14 = s14.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s14_bullets = [
    ...
    add_formatted_bullets(tb_s14.text_frame, s14_bullets, font_size=13.5)
    add_acronym_footer(s14, ["API", "OPA", "SPIFFE", "SPIRE"], y_pos=7.95, height=0.9)

    # Slide 15: E類 - 自主系統與人機協同
    s15 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s15)
    add_header(s15, "E類 - 自主系統與人機協同評測 SOP", "SECTION 3: SYSTEM-SPECIFIC AI EVALUATION SOP")
    add_icon_card(s15, 0.8, 1.6, 14.4, 6.25, "🚁", "Autonomous Systems & HMT Evaluation SOP", accent_color=BLUE)
    tb_s15 = s15.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s15_bullets = [
    ...
    add_formatted_bullets(tb_s15.text_frame, s15_bullets, font_size=13.0)
    add_acronym_footer(s15, ["RoE", "HITL", "HOTL", "HOOTL", "VBS", "EADSIM", "LVC", "ToAST", "DoDD", "MSR", "ECE", "NASA-TLX", "EEG"], y_pos=7.95, height=0.9)

    # Slide 16: F類 - 決策支援與預測分析
    s16 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s16)
    add_header(s16, "F類 - 決策支援與預測分析評測 SOP", "SECTION 3: SYSTEM-SPECIFIC AI EVALUATION SOP")
    add_icon_card(s16, 0.8, 1.6, 14.4, 6.25, "📈", "Predictive Analytics & C2 Evaluation SOP", accent_color=DARK_BLUE)
    tb_s16 = s16.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s16_bullets = [
        ...
        "3. **關鍵指標與合格門檻**：",
        "   - **Q12 概念與數據漂移監控率**：$\mathrm{Drift~Recall} \ge 0.95 \quad \wedge \quad t_{\mathrm{alarm\_latency}} \le 5\mathrm{min}$",
    ...
    add_formatted_bullets(tb_s16.text_frame, s16_bullets, font_size=13.5)
    add_acronym_footer(s16, ["PyOD", "OOD", "UQ", "MC-Dropout", "COA"], y_pos=7.95, height=0.9)
    ...
    tb_s17_m = s17.shapes.add_textbox(Inches(0.8), Inches(1.6), Inches(14.4), Inches(4.8))
    ...
    add_formatted_bullets(tf_17, m_summary, font_size=13.0)

    # Slide 17 Appendix Pointer Card
    card_ptr = s17.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(6.5), Inches(14.4), Inches(1.3))
    card_ptr.fill.solid()
    ...
    add_acronym_footer(s17, ["Acc_adv", "Acc_clean", "mAP", "MSR", "ECE", "garak", "PyOD", "ART", "RAGAS", "SOP"], y_pos=7.95, height=0.9)

    # Build metric detail slides 18-25
    metrics_list = [
    ...
    for idx, m in enumerate(metrics_list):
        ...
        add_icon_card(s_m, 0.8, 1.6, 7.0, 6.25, "📌", "指標定義與計算公式", "Metric Definition & Rendered Math", accent_color=BLUE)
        tb_l = s_m.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(6.4), Inches(1.5))
        left_bullets_top = [f"1. **核心指標定義**：{m['def']}", "2. **量化計算數學公式**："]
        add_formatted_bullets(tb_l.text_frame, left_bullets_top, font_size=13)
        s_m.shapes.add_picture(calc_img, Inches(1.3), Inches(3.8), width=Inches(6.0))

        tb_l_bot = s_m.shapes.add_textbox(Inches(1.1), Inches(4.7), Inches(6.4), Inches(3.0))
        ...
        add_icon_card(s_m, 8.2, 1.6, 7.0, 6.25, "🧪", "驗測 SOP 與合格判定門檻", "Testing SOP & Pass Threshold Math", accent_color=DARK_BLUE)
        tb_r = s_m.shapes.add_textbox(Inches(8.5), Inches(2.7), Inches(6.4), Inches(1.5))
        ...
        s_m.shapes.add_picture(thresh_img, Inches(8.7), Inches(4.5), width=Inches(6.0))

        tb_r_bot = s_m.shapes.add_textbox(Inches(8.5), Inches(5.4), Inches(6.4), Inches(2.3))
        right_bullets_bot = [f"4. **合規稽核與紀錄規範**：{m['audit']}"]
        add_formatted_bullets(tb_r_bot.text_frame, right_bullets_bot, font_size=13)

        add_acronym_footer(s_m, m['acronyms'], y_pos=7.95, height=0.9)

    # ----------------------------------------------------
    # SECTION 5: Slides 26-30 - Platforms & Infrastructure
    # ----------------------------------------------------
    ...
    add_icon_card(s26, 0.8, 1.6, 14.4, 6.25, "🏛️", "Sovereign AI Platform & 4-Tier Compute Stack", accent_color=BLUE)
    tb_s26 = s26.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s26_bullets = [
    ...
    add_formatted_bullets(tb_s26.text_frame, s26_bullets, font_size=13.5)
    add_acronym_footer(s26, ["MoE", "GGUF", "API", "SPIFFE", "OPA"], y_pos=7.95, height=0.9)

    # Slide 27: 地端 LLM 推論引擎與 Middleware 工具鏈
    ...
    add_icon_card(s27, 0.8, 1.6, 14.4, 6.25, "⚙️", "On-Prem LLM Inference Engine & Middleware", accent_color=DARK_BLUE)
    tb_s27 = s27.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s27_bullets = [
    ...
    add_formatted_bullets(tb_s27.text_frame, s27_bullets, font_size=13.5)
    add_acronym_footer(s27, ["LLM", "vLLM", "GGUF", "LoRA", "SOP"], y_pos=7.95, height=0.9)
    ...
    add_icon_card(s28, 0.8, 1.6, 14.4, 6.25, "📡", "Tactical C2 Architecture & Edge Compute Nodes", accent_color=BLUE)
    tb_s28 = s28.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s28_bullets = [
    ...
    add_formatted_bullets(tb_s28.text_frame, s28_bullets, font_size=13.5)
    add_acronym_footer(s28, ["C2", "JADC2", "API", "SPIFFE", "SPIRE", "OPA"], y_pos=7.95, height=0.9)

    # Slide 29: 戰術邊緣硬體安全與模型自毀機制
    s29 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s29)
    add_header(s29, "戰術邊緣硬體安全與模型自毀機制", "SECTION 5: EVALUATION PLATFORMS & AUTOMATED T&E")
    add_icon_card(s29, 0.8, 1.6, 14.4, 6.25, "💣", "Tactical Edge Hardware Security & Emergency Self-Destruct", accent_color=DARK_BLUE)
    tb_s29 = s29.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s29_bullets = [
        "1. **無 GPS 網狀通訊 (Mesh Network)**：在前線電戰干擾與網狀網絡環境中，無人機與邊緣設備被俘獲風險極高。",
        "2. **硬體級防篡改 (Hardware Tamper-Resistance)**：軍規 Secure Enclave / TPM 晶片與物理開箱感測電路 (Tamper Mesh)。",
        "3. **緊急模型自毀指令 (Model Self-Destruct)**：偵測到外殼異常拆解或接獲中斷信號時，在 $<100\mathrm{ms}$ 內以隨機雜訊覆寫 Flash/RAM 權重。",
        "4. **物理零化 (Zeroization)**：隨機化熔絲 (Key Fusing) 撕毀加密金鑰，徹底防止敵方逆向工程與模型洩漏。"
    ]
    add_formatted_bullets(tb_s29.text_frame, s29_bullets, font_size=13.5)
    add_acronym_footer(s29, ["GPS", "Mesh", "TPM", "RAM"], y_pos=7.95, height=0.9)

    # Slide 30: 聯邦學習 (Federated Learning) 國防保密策略
    s30 = prs.slides.add_slide(blank_layout)
    set_pure_white_bg(s30)
    add_header(s30, "聯邦學習 (Federated Learning) 國防保密策略", "SECTION 5: EVALUATION PLATFORMS & AUTOMATED T&E")
    add_icon_card(s30, 0.8, 1.6, 14.4, 6.25, "🤝", "Federated Learning Strategy & Parameter Fusion", accent_color=BLUE)
    tb_s30 = s30.shapes.add_textbox(Inches(1.1), Inches(2.7), Inches(13.8), Inches(4.8))
    s30_bullets = [
    ...
    add_formatted_bullets(tb_s30.text_frame, s30_bullets, font_size=13.5)
    add_acronym_footer(s30, ["DP", "FedAvg"], y_pos=7.95, height=0.9)
...
if __name__ == '__main__':
    generate_30_slides_master_deck()
